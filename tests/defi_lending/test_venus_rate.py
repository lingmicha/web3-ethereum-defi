...
from eth_defi.defi_lending.rates import (
    ...
    calculate_per_block_return,
    calculate_accrued_interests,
...
def test_calc_rates_using_index(web3:Web3, venus_wbnb_token: LendingToken):
    '''使用index来计算区块间的利率    
    '''
    # Get contracts
    ...
    vbnb = get_deployed_contract(web3, venus_wbnb_token.fname,
...
def test_venus_calculate_mean_return(web3:Web3, venus_wbnb_token: LendingToken):
    ...
    mean_rates_df = calculate_mean_return(df, '1D', ['deposit_apr', 'borrow_apr'], token='WBNB')
    assert mean_rates_df.loc['2022-10-30', 'borrow_apr'] == pytest.approx(1.9210414834638212, rel=1e-6)

    mean_rates_df = calculate_mean_return(df, '1D', 'borrow_apr', token='WBNB')
    assert mean_rates_df.loc['2022-10-30', 'borrow_apr'] == pytest.approx(1.9210414834638212, rel=1e-6)
...
def test_venus_deposit_depth_analysis(web3:Web3, venus_busd_token: LendingToken):

    rates = get_current_rates(web3, venus_busd_token)

    analysis = market_depth_analysis(rates, venus_busd_token)
