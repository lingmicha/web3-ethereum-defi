"""Token tax module test examples on the BSC Chain for the babydoge token
...
@pytest.fixture(scope="module")
def babydoge() -> HexAddress:
    """return hex address of the babydoge token"""
    return HexAddress(HexStr("0xc748673057861a797275CD8A068AbB95A902e8de"))
...
def test_token_tax(uniswap: UniswapV2Deployment, large_busd_holder: HexAddress, seller: HexAddress,
                   babydoge: HexAddress, busd: HexAddress):

    expected_babydoge_tax_percent: float = 0.1
    buy_amount: float = 1
...
def test_low_liquidity_exception(uniswap: UniswapV2Deployment, large_busd_holder: HexAddress, seller: HexAddress,
    ...
    with pytest.raises(SwapError):
        estimate_token_taxes(uniswap, babydoge, busd, large_busd_holder, seller, buy_amount)
