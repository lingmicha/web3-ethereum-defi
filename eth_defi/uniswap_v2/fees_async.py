...
from eth_defi.uniswap_v2.fees import BadReserves
from eth_defi.token_async import fetch_erc20_details

class AsyncUniswapV2FeeCalculator(UniswapV2FeeCalculator):
    """A helper class to estimate Uniswap fees."""

    def __init__(self, uniswap_v2: UniswapV2Deployment):
        assert uniswap_v2.web3.eth.is_async, "只支持异步RPC"
        self.deployment = uniswap_v2

    # Liften from uniswap-v2-py by Asynctomatic
    async def get_reserves(self, token_a: HexAddress, token_b: HexAddress) -> tuple[int]:
        ...
        try:
            reserve = await pair_contract.functions.getReserves().call()
    ...
    async def get_amount_out(
        ...
        *,
        ...
        for p0, p1 in pairs:
            r = await self.get_reserves(p0, p1)
...
async def estimate_buy_quantity(
    ...
    *,
    ...
    """
    fee_helper = AsyncUniswapV2FeeCalculator(uniswap)
    path = [quote_token.address, base_token.address]
    return await fee_helper.get_amount_out(quantity, path, fee=fee, slippage=slippage)
...
async def estimate_buy_price(
    ...
    *,
    ...
    if intermediate_token:
        ...
        path = [quote_token.address, base_token.address]
    return await fee_helper.get_amount_in(quantity, path, fee=fee, slippage=slippage)


async def estimate_sell_price(
    uniswap: UniswapV2Deployment,
    base_token: Contract,
    quote_token: Contract,
    quantity: int,
...
async def estimate_buy_price_decimals(
    uniswap: UniswapV2Deployment,
    ...
    *,
    ...
    """
    web3 = uniswap.web3
    quote = await fetch_erc20_details(web3, quote_token_address, raise_on_error=False)
...
def estimate_sell_price_decimals(
    ...
    *,
    ...
    (base, quote) = await asyncio.gather(
        [fetch_erc20_details(web3, base_token_address, raise_on_error=False),
         fetch_erc20_details(web3, quote_token_address, raise_on_error=False)])
    ...
    out_raw = await fee_helper.get_amount_out(quantity_raw, path, fee=fee, slippage=slippage)
...
def estimate_buy_received_amount_raw(
    ...
    *,
    ...
    return await fee_helper.get_amount_out(quantity_raw, path, fee=fee, slippage=slippage)
