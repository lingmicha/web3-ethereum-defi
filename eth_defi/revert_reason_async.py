...
async def fetch_transaction_revert_reason(
    web3: Web3,
    ...
    try:
        if use_archive_node:
            result = await web3.eth.call(replay_tx, tx.blockNumber - 1)
        else:
            result = await web3.eth.call(replay_tx)
    ...
    current_block_number = await web3.eth.block_number
